...
% Mireille : insert 
% support from Escape collaboration GRANT collaboration 
% 
...
% Mireille: find solutions for that. 
% instead : cover the needs exposed in this note. 
...
The scope of this Visibility ObsCore extension in the IVOA is very close to ObsCore itself. It's goal is to add new visibilities specific features to existing ObsCore metadata tables and expose them in ObsTAP TAP_SCHEMA. 
...
\label{sec:specificities}
   ...
   Spectral and Time axis are regularly sampled but we can have gaps between time and spectral intervals. In addition, on the spectral axis different samplings may coexist.  
    ...
    Ranges of values for fov size and resolution will achieve this task more accurately
     ...
     The quality of the data strongly depends from the distribution of the visibility measurements in the uv plane : the more complete will be the % mireille 
uv sampling plane %mireille
, the better will be the reconstructed image. The uv plane distribution can be characterized by several numbers. 
The minimal and maximum distance between measurements in the uv plane 
% mireille provide assessments for 
are important to get the actual % mireille spatial 
...
% mireille suggestion : of the image in the distance plane (sky).
...
The visualisation of the dirty beam, which is the Fourier transform of the uv sampling function gives an hint of the intrinsic quality of possible reconstruction./As maps they are not queriable. So it is questionable if links to this maps are to be exposed in the extension table or only via a DataLink service. 
          ...
          Important features are the antenna diameter (or maximum ?scale? antenna parameter) the number of antennae and the minimum and maximum distance between antennae of the array.
...
% ML suggestion:  For visibilities some of the definitions on Obscore datamodel elements need to be adjusted to fit 
...
Visibility data observations are generally splitted in several subparts with 
% mireille homogeneous spatial , time , spectral coverage intervals instead of
consistent characterization parameters 
% 
described by a single ObsCore % mireille dataset entry instead of 
record. Each of these records is considered as describing a dataset and has its own obs\_publisher\_did. It has to be unique in the virtual observatory domain.
...
To compute the eccentricity of the UV distribution a principal component analysis (PCA) with 2 components is performed over the data points sampling the UV plane to select the main axis of data scattering. 
...
%Mireille: ?? defined here in this ObsCore extension???.
.
